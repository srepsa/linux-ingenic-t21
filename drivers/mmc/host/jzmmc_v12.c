...
static void jzmmc_request_timeout(struct timer_list *t)
{
	struct jzmmc_host *host = from_timer(host, t, request_timer);
...
static void jzmmc_detect_change(struct timer_list *t)
{
	struct jzmmc_host *host = from_timer(host, t, detect_timer);
	bool			present;
	bool			present_old;
				    ...
				    const char *buf, size_t count)
{
	...
	} else if (strncasecmp(buf, "REMOVE", 6) == 0) {
...
static void __init jzmmc_host_init(struct jzmmc_host *host, struct mmc_host *mmc)
{
	...
	host->mmc = mmc;
	timer_setup(&host->request_timer, jzmmc_request_timeout,
		    0);
	mmc_add_host(mmc);
}
...
static int __init jzmmc_dma_init(struct jzmmc_host *host)
{
	...
	host->decshds[0].dma_desc = ioremap(virt_to_phys(desc_mem), PAGE_SIZE);
...
static int __init jzmmc_msc_init(struct jzmmc_host *host)
{
	...
	ret = request_irq(host->irq, jzmmc_irq, 0,
...
static int __init jzmmc_gpio_init(struct jzmmc_host *host)
{
	...
	case REMOVABLE:
		if (gpio_is_valid(card_gpio->cd.num)) {
			timer_setup(&host->detect_timer, jzmmc_detect_change, 0);
	...
	if(host->index == 0){
		*(volatile unsigned int*)(0xb0011138) = 0xfff;
		*(volatile unsigned int*)(0xb0011134) = 0x555;
	}
#endif
	return ret;
}
...
static int __init jzmmc_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		goto err_msc_init;
	jzmmc_host_init(host, mmc);  // init already here cause detect_change callback references host->mmc already...
...
{
	struct jzmmc_host *host = dev_get_drvdata(&(pdev)->dev);
...
#ifdef CONFIG_PM
static int jzmmc_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct jzmmc_host *host = dev_get_drvdata(&(pdev)->dev);
	int ret = 0;
...
static int jzmmc_resume(struct platform_device *pdev)
{
	struct jzmmc_host *host = dev_get_drvdata(&(pdev)->dev);
	int ret = 0;
